...
export function Experience({ onViewDetail }: { onViewDetail?: (id: number) => void }) {
  ...
  return (
    <section id="experience" className="py-20 px-4 relative overflow-hidden" style={{ background: '#0a0a0a' }}>
      {/* Background Elements - grayscale */}
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <div className="text-center mb-16">
          <h2 className="text-5xl md:text-6xl mb-4 font-black tracking-tight"
              style={{ background: 'linear-gradient(135deg, #d1d5db, #ffffff, #9ca3af)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
            Work Experience
          </h2>
        ...
        <div className="relative">
          ...
          <div className="hidden md:block absolute left-1/2 transform -translate-x-1/2 w-1 h-full rounded-full"
               style={{ background: 'linear-gradient(to bottom, #e5e7eb, #9ca3af, #e5e7eb)', boxShadow: '0 0 8px rgba(255,255,255,0.4)' }}></div>
                ...
                >
                  ...
                  <div className={`w-full md:w-5/12 ${isEven ? 'md:text-right' : 'md:text-left'}`}>
                    <div 
                      ...
                      className="group bg-black/60 backdrop-blur-sm rounded-3xl p-8 shadow-xl border border-white/10 hover:border-white/20 hover:shadow-2xl hover:scale-[1.02] transition-all duration-500 cursor-pointer"
                    >
                      {/* Header */}
                      <div className={`flex items-start gap-4 mb-6 ${isEven ? 'md:flex-row-reverse' : ''}`}>
                        <div className={`w-16 h-16 bg-gradient-to-br ${getGradientClasses(exp.color)} rounded-2xl flex items-center justify-center shadow-lg flex-shrink-0 group-hover:scale-110 transition-transform duration-300`}>
                          <Icon className="w-8 h-8 text-white" />
                        </div>
                        <div className={`flex-1 ${isEven ? 'md:text-right' : 'md:text-left'}`}>
                          <h3 className="text-2xl text-white/90 mb-1">{exp.title}</h3>
                      ...
                      <div className={`flex flex-wrap gap-3 mb-6 ${isEven ? 'md:justify-end' : 'md:justify-start'}`}>
                        <div className="flex items-center gap-2 px-4 py-2 bg-white/5 text-gray-300 rounded-xl text-sm border border-white/10">
                  ...
                  <div className="hidden md:flex w-2/12 justify-center items-center z-10">
                    <div className="relative">
                      <div className={`w-6 h-6 bg-gradient-to-br ${getGradientClasses(exp.color)} rounded-full border-4 border-white shadow-lg animate-pulse`}></div>
                      <div className={`absolute inset-0 w-6 h-6 bg-gradient-to-br ${getGradientClasses(exp.color)} rounded-full animate-ping opacity-75`}></div>
                    </div>
                  </div>
        ...
        <div className="mt-16 grid grid-cols-2 md:grid-cols-4 gap-6">
          {[
            ...
            return (
              <div
                key={index}
                className="bg-black/60 backdrop-blur-sm rounded-2xl p-6 text-center border border-white/10 hover:shadow-xl transition-all duration-300 hover:scale-105"
              >
